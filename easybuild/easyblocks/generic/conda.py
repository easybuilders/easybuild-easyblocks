...
class Conda(Binary):
    ...
    def install_step(self):
        ...
        if self.cfg['environment_file'] or self.cfg['remote_environment']:
            ...
            cmd = "%s conda env create --force %s -p %s %s" % (self.cfg['preinstallopts'], env_spec, self.installdir, self.cfg['installopts'])
            run_cmd(cmd, log_all=True, simple=True)

        else:
            cmd = "%s conda create --force -y -p %s %s" % (self.cfg['preinstallopts'], self.installdir, self.cfg['installopts'])
            run_cmd(cmd, log_all=True, simple=True)
