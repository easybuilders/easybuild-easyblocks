...
class EB_QuantumESPRESSO_autotools(ConfigureMake):
    """Support for building and installing Quantum ESPRESSO using Autotools."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        if LooseVersion(self.version) >= LooseVersion("7.3.1"):
            raise EasyBuildError(
                "QuantumESPRESSO 7.3.1 and later are not supported with the autotools-based QuantumESPRESSO easyblock, "
                "use the (default) EB_QuantumESPRESSO (CMakeMake) easyblock instead."
                )
