...
class EB_CP2K(EasyBlock):
    ...
    def configure_step(self):
        ...
        if self.cfg['type'] not in known_types:
            raise EasyBuildError(
                "Unknown build type specified: '%s', known types are %s", self.cfg['type'], known_types
            )
        if cp2k_version >= LooseVersion('2024') and self.cfg['type'] == 'popt':
            self.cfg['type'] = 'psmp'
            setvar('OMP_NUM_THREADS', '1')
            self.log.debug('In 2024+ popt maps to psmp with OMP_NUM_THREADS=1')
        ...
        if get_software_root('imkl'):
            ...
            fftw_loaded = get_software_root('FFTW') or 'fftw3' in os.getenv('LIBFFT', '')
            if fftw_loaded:
    ...
    def prepmodinc(self):
        ...
        if imkl:
            ...
            if isinstance(self.cfg["modinc"], list):
                ...
                raise EasyBuildError(
                    "prepmodinc: Please specify either a boolean value or a list of files in modinc " "(found: %s).",
                    self.cfg["modinc"],
                )
    ...
    def configure_common(self):
        ...
        if hasattr(self.toolchain, 'MPI_FAMILY') and self.toolchain.MPI_FAMILY is not None:
            ...
            for mpi2lib in mpi2libs:
                if get_software_root(mpi2lib):
                    mpi2 = True
                    self.log.debug("Determined MPI2 compatibility based on loaded MPI module: %s" % mpi2lib)
                else:
                    self.log.debug("MPI-2 supporting MPI library %s not loaded." % mpi2lib)

        if not mpi2:
            raise EasyBuildError("CP2K needs MPI-2, no known MPI-2 supporting library loaded?")

        ldflags = os.getenv('LDFLAGS') or ''
        fflags = os.getenv('FFLAGS') or ''
        mpicc = os.getenv('MPICC') or getattr(self.toolchain, 'cc', 'mpicc')
        mpif90 = os.getenv('MPIF90') or getattr(self.toolchain, 'fc', 'mpif90')
        ...
        options = {
            ...
            'INCS': '',
            'CFLAGS': '-O3 -fopenmp -ftree-vectorize -march=native -fno-math-errno -std=c11 $(FPIC) $(DEBUG) '
            '$(INCS) $(DFLAGS) %s' % self.cfg['extracflags'],
            'DEBUG': self.debug,
            'FREE': '',
        ...
        # Make LIBXSMM visible for all CP2K versions (2023.x and 2024+)
        if self.libsmm_path:
            # headers / .mod
            ...
            lib64 = os.path.join(self.libsmm_path, 'lib64')
        ...
        options.update(
            {
                ...
                'LDFLAGS': options['LDFLAGS'] + ' $(INCFLAGS) ',
    ...
    def configure_FFTW3(self, options):
        ...
        if cp2k_version >= LooseVersion('2024'):
            ...
            libdir = os.getenv('FFT_LIB_DIR')
            incdir = os.getenv('FFT_INC_DIR')
            if libdir:
                options['LDFLAGS'] += f' -L{libdir}'
            if incdir:
                options['INCS'] += f' -I{incdir}'
        else:
            options.update(
                {
                    'FFTW_INC': os.getenv('FFT_INC_DIR', ''),
                    'FFTW3INC': os.getenv('FFT_INC_DIR', ''),
                    'FFTW3LIB': os.getenv('FFT_LIB_DIR', ''),
                }
            )
            libfft = os.getenv('LIBFFT_MT', '') if self.cfg['type'] == 'psmp' else os.getenv('LIBFFT', '')
    ...
    def test_step(self):
        ...
        if self.cfg['runtest']:
            ...
            def test_report(test_result):
                ...
                if not res:
                    raise EasyBuildError(
                        "Finding number of %s tests in regression test summary failed", test_result.lower()
                    )
                else:
    ...
    def sanity_check_step(self):
        ...
        custom_paths = {'files': ["bin/%s.%s" % (x, cp2k_type) for x in ["cp2k", "cp2k_shell"]], 'dirs': ["tests"]}
        if self.cfg['library']:
