...
##
"""
EasyBuild support for AmberTools, implemented as an easyblock

Based on the original Amber easyblock
...
Modified for AmberTools by Adam Huffman (Francis Crick Institute)
"""
import os
...
class EB_AmberTools(ConfigureMake):
    """Easyblock for building and installing AmberTools"""
    ...
    def extra_options(extra_vars=None):
        ...
        extra_vars.update({
            ...
            'patchlevels': ["latest", "AmberTools updates to be applied", CUSTOM],
            # The following is necessary because some patches to the AmberTools update
            ...
            'patchruns': [1, "Number of times to run AmberTools' update script before building", CUSTOM],
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(EB_AmberTools, self).__init__(*args, **kwargs)
    ...
    def extract_step(self):
        ...
        super(EB_AmberTools, self).extract_step()

    def patch_step(self, *args, **kwargs):
        """Patch AmberTools using 'update_amber' tool, prior to applying listed patch files (if any)."""
        ...
        if self.cfg['patchlevels'] == "latest":
            ...
            for (tree, patch_level) in zip(['AmberTools'], self.cfg['patchlevels']):
                if patch_level == 0:
                    continue
		# patch_level truncates double digit integers to the first digit
		# hence use patchlevels directly, for now
                cmd = "./update_amber --update-to %s/%s" % (tree, self.cfg['patchlevels'])
                # Run as many times as specified. It is the responsibility
                # of the easyconfig author to get this right.
                for _ in range(self.cfg['patchruns']):
                    run_cmd(cmd, log_all=True)

        super(EB_AmberTools, self).patch_step(*args, **kwargs)

    def configure_step(self):
        """Configuring AmberTools is done in install step."""
        pass

    def build_step(self):
        """Building AmberTools is done in install step."""
        pass

    def test_step(self):
        """Testing AmberTools build is done in install step."""
        pass

    def install_step(self):
        """Custom build, test & install procedure for AmberTools."""
        ...
        for flag, testrule in build_targets:
            ...
            # note: not 'build'
            super(EB_AmberTools, self).install_step()
    ...
    def sanity_check_step(self):
        """Custom sanity check for AmberTools."""
        binaries = ['cpptraj', 'sander', 'tleap']
        #if self.with_cuda:
            #binaries.append('pmemd.cuda')
            #if self.with_mpi:
                #binaries.append('pmemd.cuda.MPI')
        if self.with_mpi:
            binaries.extend(['cpptraj.MPI', 'sander.MPI'])

        custom_paths = {
            'files': [os.path.join(self.installdir, 'bin', binary) for binary in binaries],
            'dirs': [],
        }
        super(EB_AmberTools, self).sanity_check_step(custom_paths=custom_paths)

    def make_module_extra(self):
        """Add module entries specific to AmberTools"""
        txt = super(EB_AmberTools, self).make_module_extra()
